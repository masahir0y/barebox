...
#define SG_MEMCONF_CH1_SZ_2G		((0x1 << 11) | (0x02 << 2))
...
static inline void sg_set_pinsel(int n, int value, int field_width, int stride)
{
	unsigned long reg;
	int shift;
	u32 mask, tmp;

	reg = SG_PINCTRL_BASE + (field_width * n / 32) * stride;
	shift = field_width * n % 32;
	mask = ~(((1L << field_width) - 1) << shift);
